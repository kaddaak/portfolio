...
- The Raspberry stays in the middle so room traffic is coordinated in one place instead of peer traffic between registers
...
- Each register loads the active desktop peers from the Raspberry and derives one shared room plus direct rooms between registers
...
- Broadcast communication to the full register cluster plus direct conversations between registers
