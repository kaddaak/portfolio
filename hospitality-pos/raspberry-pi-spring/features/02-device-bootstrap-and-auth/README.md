...
Bootstrap is the site-enrollment step.
...
After bootstrap, that installation is treated as that register inside the site. Communication with the Raspberry node runs over certificate-pinned TLS, and the install is no longer just another untrusted LAN client.
...
Protected APIs are device-gated first. Operator login does not establish trust from scratch; it upgrades an already trusted device session by adding user identity on top of device identity.

That keeps the model split into:
...
This lets the Raspberry Pi decide which installs are actually part of the site register cluster. Unbootstrapped installs are ignored, register ownership is explicit, TLS trust is established during enrollment, and every protected workflow runs on top of trusted device context instead of anonymous LAN access.
