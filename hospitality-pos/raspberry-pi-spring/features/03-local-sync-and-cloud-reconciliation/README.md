...
The Raspberry Pi hydrates from the cloud once, reconciles incrementally after that, and serves local clients from a single snapshot of site state.
...
- Clients keep their previous section data for `incompatible` and `error` sections, preserving one coherent snapshot while failed sections recover.
